...
from config import (
    ACTIVITY_WEIGHTS,
    CHLOROPHYLL_A_RED_UG_L,
    CHLOROPHYLL_A_YELLOW_UG_L,
    RISK_THRESHOLDS,
    TURBIDITY_RED_NTU,
    TURBIDITY_YELLOW_NTU,
)
...
def score_risk(
    ...
    observations: Iterable[HABObservation] = (),
) -> dict[str, ActivityRisk]:
    """Combine quality layers and optional field observations into activity risk."""
    turbidity_signal = _ramp(
        metrics.turbidity_p90_ntu, TURBIDITY_YELLOW_NTU, TURBIDITY_RED_NTU
    )
    chlorophyll_signal = _ramp(
        metrics.chlorophyll_a_p90_ug_l,
        CHLOROPHYLL_A_YELLOW_UG_L,
        CHLOROPHYLL_A_RED_UG_L,
    )
    if metrics.chlorophyll_a_threshold_exceeded:
        chlorophyll_signal = max(chlorophyll_signal, 1.0)
    observation_signal = max(
        (observation.severity for observation in observations), default=0.0
    )
    output: dict[str, ActivityRisk] = {}
    for activity, weights in ACTIVITY_WEIGHTS.items():
        score = 0.0
        for name, signal in (
            ("turbidity", turbidity_signal),
            ("chlorophyll", chlorophyll_signal),
        ...
        rationale = (
            f"{label} screening signal for {activity}: "
            f"turbidity={turbidity_signal:.2f}, "
            f"chlorophyll={chlorophyll_signal:.2f}, "
            f"field-observation={observation_signal:.2f}."
        )
